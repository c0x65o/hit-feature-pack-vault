    ...
    try {
        ...
        if (aclAccessibleVaultIds.size > 0) {
            const allowedShared = await db
                .select({ id: vaultVaults.id })
                .from(vaultVaults)
                .where(and(eq(vaultVaults.type, 'shared'), inArray(vaultVaults.id, Array.from(aclAccessibleVaultIds))));
            aclAccessibleVaultIds.clear();
            for (const v of allowedShared)
                aclAccessibleVaultIds.add(v.id);
        }
        ...
        if (!type || type === 'shared') {
            const shared = items.filter((v) => String(v?.type) === 'shared');
            if (shared.length > 1) {
                shared.sort((a, b) => {
                ...
                items = items.filter((v) => String(v?.type) !== 'shared' || String(v?.id) === String(keepId));
            }
        }
