    ...
    try {
        ...
        if (isAdmin) {
            ...
            for (const acl of vaultAclsList) {
                ...
                if (sharedVaultIds.has(acl.resourceId))
        ...
        if (accessibleVaultIds.size > 0 && accessibleFolderIds.size > 0) {
            accessConditions.push(or(inArray(vaultItems.vaultId, Array.from(accessibleVaultIds)), inArray(vaultItems.folderId, Array.from(accessibleFolderIds))));
        }
        ...
        const items = await db
            ...
            .from(vaultItems)
            .where(and(...accessConditions, or(like(vaultItems.title, `%${query}%`), like(vaultItems.username, `%${query}%`), like(vaultItems.url, `%${query}%`))))
            .limit(50);
        return NextResponse.json({ items });
    }
    catch (error) {
