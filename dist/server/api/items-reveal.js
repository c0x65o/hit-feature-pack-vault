    ...
    try {
        ...
        try {
            ...
            if (!item.secretBlobEncrypted || !item.secretBlobEncrypted.trim()) {
                ...
                return NextResponse.json({ error: 'Item secret data is missing or corrupted' }, { status: 500 });
            }
            ...
            if (parts.length !== 3) {
                ...
                return NextResponse.json({ error: 'Item secret data is in an invalid format. This may be due to a migration issue.' }, { status: 500 });
            }
            ...
            return NextResponse.json({ error: `Failed to decrypt item: ${errorMessage}` }, { status: 500 });
        }
    }
    catch (error) {
