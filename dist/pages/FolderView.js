...
export function FolderView({ folderId, onNavigate }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsxs(Page, { title: folder?.name || 'Folder not found', description: folder ? `${folder.path} • ${items.length} items` : '', breadcrumbs: breadcrumbs, onNavigate: navigate, actions: folder && folder.canShare ? (_jsxs(Button, { onClick: () => setAclModalOpen(true), variant: "secondary", size: "sm", children: [_jsx(Users, { size: 16, className: "mr-2" }), "Manage Access"] })) : undefined, children: [error && (_jsx(Alert, { variant: "error", title: "Error loading folder", children: error.message })), !folder && (_jsx(Card, { children: _jsx("div", { className: "p-6 text-center text-muted-foreground", children: "Folder not found" }) })), folder && items.length === 0 && (_jsx(Card, { children: _jsx("div", { className: "p-6 text-center text-muted-foreground", children: "This folder is empty." }) })), folder && items.length > 0 && (_jsx("div", { className: "border rounded-lg overflow-hidden", children: items.map((item, index) => {
