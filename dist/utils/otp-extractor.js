...
const OTP_PATTERNS = [
    ...
    { regex: /(?:verification|verify|security|auth|otp|pin|code|password)\s*(?:code|number|pin)?[:\s]+([A-Z0-9]{5,8})/i, name: 'keyword-prefix-alphanumeric', confidence: 'high' },
    { regex: /([A-Z0-9]{5,8})\s+(?:is|es)\s+(?:your|tu|the)\s+(?:verification|verify|security|auth|otp|pin|code|password)/i, name: 'keyword-suffix-alphanumeric', confidence: 'high' },
    // Common patterns with explicit OTP keywords (numeric, 5-8 chars minimum)
    { regex: /(?:verification|verify|security|auth|otp|pin|code|password)\s*(?:code|number|pin)?[:\s]+(\d{5,8})/i, name: 'keyword-prefix', confidence: 'high' },
    { regex: /(\d{5,8})\s+(?:is|es)\s+(?:your|tu|the)\s+(?:verification|verify|security|auth|otp|pin|code|password)/i, name: 'keyword-suffix', confidence: 'high' },
    // "Your code is X" style (alphanumeric, 5-8 chars minimum)
    { regex: /(?:your|tu|the)\s+(?:verification\s+)?code\s+(?:is|es)[:\s]+([A-Z0-9]{5,8})/i, name: 'your-code-is-alphanumeric', confidence: 'high' },
    // "Your code is X" style (numeric, 5-8 chars minimum)
    { regex: /(?:your|tu|the)\s+(?:verification\s+)?code\s+(?:is|es)[:\s]+(\d{5,8})/i, name: 'your-code-is', confidence: 'high' },
    // "code is X" or "verification code is X" (without "your") - numeric, 5-8 chars minimum
    { regex: /(?:verification\s+)?code\s+(?:is|es)\s+(\d{5,8})/i, name: 'code-is-numeric', confidence: 'high' },
    // "code is X" or "verification code is X" (without "your") - alphanumeric, 5-8 chars minimum
    { regex: /(?:verification\s+)?code\s+(?:is|es)\s+([A-Z0-9]{5,8})/i, name: 'code-is-alphanumeric', confidence: 'high' },
    { regex: /(?:code|código)[:\s]+(\d{5,8})/i, name: 'code-colon', confidence: 'high' },
    // "Code: X" or "OTP: X" patterns (alphanumeric, 5-8 chars minimum)
    { regex: /\botp[:\s]+([A-Z0-9]{5,8})\b/i, name: 'otp-colon-alphanumeric', confidence: 'high' },
    { regex: /\bpin[:\s]+([A-Z0-9]{5,8})\b/i, name: 'pin-colon-alphanumeric', confidence: 'high' },
    // "Code: X" or "OTP: X" patterns (numeric, 5-8 chars minimum)
    { regex: /\botp[:\s]+(\d{5,8})\b/i, name: 'otp-colon', confidence: 'high' },
    { regex: /\bpin[:\s]+(\d{5,8})\b/i, name: 'pin-colon', confidence: 'high' },
    // "Use X to verify" patterns (alphanumeric, 5-8 chars minimum)
    { regex: /use\s+([A-Z0-9]{5,8})\s+(?:to|for)\s+(?:verify|confirm|validate)/i, name: 'use-to-verify-alphanumeric', confidence: 'high' },
    // "Use X to verify" patterns (numeric, 5-8 chars minimum)
    { regex: /use\s+(\d{5,8})\s+(?:to|for)\s+(?:verify|confirm|validate)/i, name: 'use-to-verify', confidence: 'high' },
    // "Enter X" patterns (alphanumeric, 5-8 chars minimum)
    { regex: /enter[:\s]+([A-Z0-9]{5,8})/i, name: 'enter-code-alphanumeric', confidence: 'high' },
    // "Enter X" patterns (numeric, 5-8 chars minimum)
    { regex: /enter[:\s]+(\d{5,8})/i, name: 'enter-code', confidence: 'high' },
    // Codes with dashes/spaces (e.g., "123-456" or "123 456") - total must be at least 5 digits
    { regex: /code[:\s]+(\d{3,4}[-\s]\d{3,4})/i, name: 'code-with-dash', confidence: 'high' },
