...
export function FolderAclModal({ folderId, isOpen, onClose, onUpdate }) {
    ...
    const fetchPrincipals = useMemo(() => createFetchPrincipals({
        isAdmin: isAdminUser,
        extraPrincipals: async (type, search) => {
            if (type !== 'group' || !isAdminUser)
                return [];
