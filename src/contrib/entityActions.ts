...
async function revealVaultItemSecrets(itemId: string): Promise<{
...
async function generateTotpCodeWeb(secretBase32: string): Promise<string> {
  ...
  const cryptoAny: any = crypto as any;
  if (!cryptoAny?.subtle) throw new Error('WebCrypto is not available');
  const hmacKey = await cryptoAny.subtle.importKey(
    'raw',
    keyBytes,
...
export const actionHandlers: Record<string, (ctx: PackActionHandlerContext) => void | Promise<void>> = {
  'vault.copyUsername': async ({ record }) => {
    const username = String(record?.username || '').trim();
    if (!username) throw new Error('No username on this item');
  ...
  'vault.copyPassword': async ({ record }) => {
    ...
    if (!value) throw new Error('No secret/password found for this item');
  ...
  'vault.generateTotp': async ({ record }) => {
    ...
    if (!secret) throw new Error('No TOTP secret configured for this item');
