...
type WebhookAuthDebug =
  | { method: 'twilio_signature'; providedPrefix: string | null; expectedPrefix: null; providedValue: string | null }
...
function getAuthDebugFromRequest(request: NextRequest, expectedPrefix: string | null): WebhookAuthDebug {
  ...
  if (twilioSignature) {
    return {
      ...
      providedPrefix: twilioSignature.substring(0, 8) || null,
      expectedPrefix: null,
      providedValue: twilioSignature,
    };
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!fromNumber || !toNumber || !messageBody) {
      ...
      if (!fromNumber) missingFields.push('from/From');
      if (!toNumber) missingFields.push('to/To');
      if (!messageBody) missingFields.push('body/Body');
    ...
    // Attach publish debug details to the webhook log body (without leaking secrets).
