...
export async function GET(request: NextRequest) {
  try {
    ...
    const userPersonalVaults = await db
      ...
      .from(vaultVaults)
      .where(and(
        eq(vaultVaults.ownerUserId, user.sub),
    ...
    const accessibleVaultIds = new Set<string>(userPersonalVaultIds);
    ...
    if (vaultId && !accessibleVaultIds.has(vaultId) && accessibleFolderIds.size > 0) {
      ...
      const foldersInThisVault = await db
        ...
        .where(
          and(
            eq(vaultFolders.vaultId, vaultId),
            inArray(vaultFolders.id, Array.from(accessibleFolderIds))
          )
        );
      
      filteredAccessibleFolderIds = new Set(foldersInThisVault.map((f: { id: string }) => f.id));
    }
    ...
    if (accessibleVaultIds.size > 0 && filteredAccessibleFolderIds.size > 0) {
      ...
      conditions.push(
        or(
          inArray(vaultFolders.vaultId, Array.from(accessibleVaultIds)),
          inArray(vaultFolders.id, Array.from(filteredAccessibleFolderIds))
        )!
      );
  ...
  try {
    ...
    if (!body.parentId) {
      ...
      const [parent] = await db
        ...
        .from(vaultFolders)
        .where(and(
          eq(vaultFolders.id, body.parentId),
          eq(vaultFolders.vaultId, body.vaultId)
        ))
        .limit(1);
