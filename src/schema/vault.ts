 ...
 */
export const vaultVaults = pgTable(
  "vault_vaults",
  {
...
export const vaultFolders = pgTable(
  "vault_folders",
  {
    ...
    parentId: uuid("parent_id").references((): AnyPgColumn => vaultFolders.id, { onDelete: "cascade" }), // Nullable for root folders
    name: varchar("name", { length: 255 }).notNull(),
  ...
  (table) => ({
    ...
    vaultParentNameIdx: unique("vault_folders_vault_parent_name_unique").on(
      table.vaultId,
      table.parentId,
      table.name
 ...
 */
export const vaultAcls = pgTable(
  "vault_acls",
  {
    id: uuid("id").primaryKey().defaultRandom(),
  ...
  (table) => ({
    ...
    resourcePrincipalIdx: unique("vault_acls_resource_principal_unique").on(
      table.resourceType,
      table.resourceId,
      table.principalType,
      table.principalId
 ...
 */
export const vaultStaticGroups = pgTable(
  "vault_static_groups",
  {
    id: uuid("id").primaryKey().defaultRandom(),
 ...
 */
export const vaultGroupMembers = pgTable(
  "vault_group_members",
  {
    id: uuid("id").primaryKey().defaultRandom(),
 ...
 */
export const vaultSmsNumbers = pgTable(
  "vault_sms_numbers",
  {
    id: uuid("id").primaryKey().defaultRandom(),
 ...
 */
export const vaultSmsMessages = pgTable(
  "vault_sms_messages",
  {
    id: uuid("id").primaryKey().defaultRandom(),
 ...
 */
export const vaultWebhookLogs = pgTable(
  "vault_webhook_logs",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    receivedAt: timestamp("received_at").defaultNow().notNull(),
    ...
    body: jsonb("body").$type<Record<string, any>>(), // Request body (sanitized)
